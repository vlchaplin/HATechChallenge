...
ClusterAnalysisResult ClusterAnalysis::analyze(const Eigen::MatrixXd & data, const std::vector<std::string> & contigs, const std::map<std::string, unsigned> & contigSizes)
{
	...
	VLOG << "Skipping Testing for multi-modality..." << std::endl;
	res.isMultiModal = 1;
	//				   cSne.isMultiModal(0, 0.001) ||
	...
	res.clustCC = cSne.connComponents(9); // Changed to 27 nearest neighbors
	res.numClustCC = res.clustCC.numClusters;
	res.hasSeparatedComponents = res.numClustCC > 1;
	// How many clusters?
	VLOG << "Number of clusters: " << res.numClustCC << std::endl;
	// What are the breakdowns?
	// VLOG << "Cluster sizes: " << res.clustCC << std::endl;
